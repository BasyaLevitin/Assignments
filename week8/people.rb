...
# 1. Let's start by creating two classes: one called Student(#I'll name it Child, so I don't ovelap on names with other files) and another called Instructor.
...
# 4. Both the instructor and the student have names. We know that instructors and students are both people. Create a parent Person(#I'll name it human so no overlap with other files) class that contains the attribute **name** and an initializer to set the name.
